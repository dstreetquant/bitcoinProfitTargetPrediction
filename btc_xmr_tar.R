...
maxHighDiffPercOvTxnCost <- (maxHighDiffPerc - txnCostinPerc)
logitTarget <- ifelse(maxHighDiffPercOvTxnCost$Open > profitTarPerc, 1, 0)
colnames(logitTarget) <- c("TARGET")
logitDF <- merge(logitDF, logitTarget)
dataset <- logitDF
dataset <- na.omit(dataset)
#dataset <- dataset[1000:5000, ]  #testing
dataset <- dataset[200000:nrow(dataset), ]  #testing
...
y_pred_logit = ifelse(logit_pred > 0.4, 1, 0)
...
print(acLogit)
sucHitLogit = cmLogit[2,2]/(cmLogit[1,2] + cmLogit[2,2]) * 100
print(sucHitLogit)
profitLogit = (cmLogit[2,2] * profitTarPerc) - (cmLogit[1,2] * (profitTarPerc/2))
print(profitLogit)
numDaysLogit = (nrow(test_set) * 5) / (24*60)
print(numDaysLogit)

###################### ANN #######################################
library(caTools)
set.seed(123)
annDF <- buyDF[, -ncol(buyDF)]
maxHighDiffPercOvTxnCost <- (maxHighDiffPerc - txnCostinPerc)
annTarget <- ifelse(maxHighDiffPercOvTxnCost$Open > profitTarPerc, 1, 0)
colnames(annTarget) <- c("TARGET")
annDF <- merge(annDF, annTarget)
dataset <- annDF
dataset <- na.omit(dataset)
#dataset <- dataset[1000:5000, ]  #testing
dataset <- dataset[1:10, ]  #testing
split = sample.split(dataset$TARGET, SplitRatio = 0.75)
training_set = subset(dataset, split == TRUE)
test_set = subset(dataset, split == FALSE)
training_set[, -ncol(training_set)] = scale(training_set[, -ncol(training_set)])
...
library(h2o)
h2o.init(nthreads = -1)
annModel = h2o.deeplearning(y = 'TARGET',
                         training_frame = as.h2o(training_set),
                         activation = 'Rectifier',
                         hidden = c(5,5),
                         epochs = 100,
                         train_samples_per_iteration = -2)
pred_ann = h2o.predict(annModel, newdata = as.h2o(test_set[-ncol(test_set)]))
y_pred_ann = (pred_ann > 0.5)
cmAnn = table(test_set[, ncol(test_set)], y_pred_ann)
acAnn = (cmAnn[1,1] + cmAnn[2,2])/(cmAnn[1,1] + cmAnn[1,2] + cmAnn[2,1] + cmAnn[2,2]) * 100
print(acAnn)
sucHitAnn = cmAnn[2,2]/(cmAnn[1,2] + cmAnn[2,2]) * 100
print(sucHitAnn)
profitAnn = (cmAnn[2,2] * profitTarPerc) - (cmAnn[1,2] * (profitTarPerc/2))
print(profitAnn)
numDaysAnn = (nrow(test_set) * 5) / (24*60)
print(numDaysAnn)
